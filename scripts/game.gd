...
func _ready():
	...
	if scene_path.find("Easy") != -1:
		...
		current_difficulty = "easy" # fallback
	...
	print("🎮 Player starts at island:", current_island)
	...
	if not bridge_manager.has_any_bridge(current_island):
		gameover_popup.popup_centered()
		print("⚠️ Player is stuck! Game over.")
...
func _on_check_pressed() -> void:
	if is_puzzle_solvable():
		show_message("✅ Puzzle is solvable from current start!")
	else:
		show_message("⚠️ Puzzle is unsolvable. Fixing bridges...")
		_fix_bridges_solvability()
		show_message("✅ Puzzle is now solvable!")
...
func _fix_bridges_solvability() -> void:
	while not is_puzzle_solvable():
		bridge_manager.create_bridges_between(island_nodes)
