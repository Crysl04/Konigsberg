...
@onready var sounds = {
	...
	&"wrong_move" : AudioStreamPlayer.new(),  
	&"game_over" : AudioStreamPlayer.new(),       
	&"game_complete" : AudioStreamPlayer.new(),   
	&"correct" : AudioStreamPlayer.new(),  
...
func _ready() -> void:
	...
	for i in sounds.keys():
		sounds[i].stream = load("res://assets/sound_effects/" + str(i) + ".ogg")
...
func install_sounds(node: Node) -> void:
	for i in node.get_children():
		if i is Button:
			...
			i.button_down.connect( ui_sfx_play.bind(&"UI_Click") )
